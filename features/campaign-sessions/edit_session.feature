...
Feature: Edit session
  ...
  Scenario: Editing a session updates its details and re-sorts the list
    ...
    Then I see session "Session 1" named "The Dark Departure" in the sessions list
    And the "Session 1" session card shows the updated cover art
    And the "Session 1" session card shows "The party leaves the village" as the description snippet
  ...
  Scenario: A failed session save shows an error without changing the list
    ...
    And I tap Edit on the "Session 1" session card
