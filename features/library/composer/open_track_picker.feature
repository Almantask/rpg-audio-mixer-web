...
Feature: Open track picker in Category Composer
  ...
  Scenario: Each track card shows a selection checkbox and file metadata
    Given the Track Picker modal is open for "Level I" in "Weather"
    And the soundscape library has "Thunderous Downpour"
    Then the "Thunderous Downpour" card displays a selection checkbox
    And the "Thunderous Downpour" card shows format, channel, and duration metadata
    And the "Thunderous Downpour" card does not display a + button
