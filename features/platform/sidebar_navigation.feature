...
Feature: Sidebar navigation
  ...
  So that I can move quickly between Home, Campaigns, Scenes, Library, Credits, and Trash.
  ...
  Scenario Outline: Sidebar navigates to the correct screen
    ...
    Examples:
      ...
      | Home screen     | Campaigns | Active Campaigns screen                     |
  ...
  Scenario: Campaign session drill-down keeps Campaigns highlighted in the sidebar
    Given I have session "Session 1" in "Curse of Strahd"
    And I am viewing Session Scenes for "Session 1"
    Then the "Campaigns" sidebar item appears highlighted in gold
  ...
  Scenario: Active Scene opened from a campaign session keeps Campaigns highlighted
    Given I have session "Session 1" in "Curse of Strahd"
    And I am viewing Session Scenes for "Session 1"
    When I open the Active Scene for "Tavern"
    Then the "Campaigns" sidebar item appears highlighted in gold

  Scenario: The floating Open menu control reveals the sidebar on narrow viewports
  ...
  Scenario: Close menu in the open sidebar hides it on narrow viewports
    Given the viewport is narrow enough to collapse the sidebar
    And the sidebar is visible
    When I tap Close menu in the sidebar
